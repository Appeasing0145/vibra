...
DownsampleFunc Downsampler::getDownsampleFunc(bool is_signed,
  ...
  static std::map<std::tuple<bool, std::uint32_t, std::uint32_t>,
      ...
      func_map{
          ...
          {std::make_tuple(false, 32, 2),
           &Downsampler::floatStereoToMono<float>},
          {std::make_tuple(false, 32, 3),
           &Downsampler::floatMultiToMono<float>},
          {std::make_tuple(false, 64, 1),
           &Downsampler::floatMonoToMono<double>},
          {std::make_tuple(false, 64, 2),
           &Downsampler::floatStereoToMono<double>},
          {std::make_tuple(false, 64, 3),
           &Downsampler::floatMultiToMono<double>},
      };
...
void Downsampler::signedMonoToMono(LowQualityTrack* dst, const void* src,
  ...
  for (std::uint32_t i = 0; i < new_sample_count; i++) {
    index = static_cast<std::uint32_t>(i * downsample_ratio) * width * channels;
    dst->at(i) =
...
void Downsampler::signedStereoToMono(LowQualityTrack* dst, const void* src,
  ...
  for (std::uint32_t i = 0; i < new_sample_count; i++) {
    index = static_cast<std::uint32_t>(i * downsample_ratio) * width * channels;
    const LowQualitySample sample1 =
        GETSAMPLE64(width, src, index) >> (64 - LOW_QUALITY_SAMPLE_BIT_WIDTH);
    const LowQualitySample sample2 = GETSAMPLE64(width, src, index + width) >>
                                     (64 - LOW_QUALITY_SAMPLE_BIT_WIDTH);
...
void Downsampler::signedMultiToMono(LowQualityTrack* dst, const void* src,
  ...
  for (std::uint32_t i = 0; i < new_sample_count; i++) {
    ...
    for (std::uint32_t k = 0; k < channels; k++) {
      index =
          static_cast<std::uint32_t>(i * downsample_ratio) * width * channels;
      collected_sample += GETSAMPLE64(width, src, index + k * width) >>
                          (64 - LOW_QUALITY_SAMPLE_BIT_WIDTH);
    }
...
void Downsampler::floatStereoToMono(LowQualityTrack* dst, const void* src,
  ...
  for (std::uint32_t i = 0; i < new_sample_count; i++) {
    ...
    temp_sample2 =
        GETSAMPLE64(width, src, index + width) >> (64 - sizeof(T) * 8);
    temp_float_sample1 = *reinterpret_cast<T*>(&temp_sample1);
    temp_float_sample2 = *reinterpret_cast<T*>(&temp_sample2);
    dst->at(i) = LowQualitySample((temp_float_sample1 + temp_float_sample2) /
                                  2 * LOW_QUALITY_SAMPLE_MAX);
  }
}
...
void Downsampler::floatMultiToMono(LowQualityTrack* dst, const void* src,
  ...
  for (std::uint32_t i = 0; i < new_sample_count; i++) {
    ...
    for (std::uint32_t k = 0; k < channels; k++) {
      temp_sample =
          GETSAMPLE64(width, src, index + k * width) >> (64 - sizeof(T) * 8);
      collected_sample += *reinterpret_cast<T*>(&temp_sample);
    }
    dst->at(i) =
        LowQualitySample(collected_sample / channels * LOW_QUALITY_SAMPLE_MAX);
  }
}
