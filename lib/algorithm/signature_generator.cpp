...
SignatureGenerator::SignatureGenerator()
    : input_pending_processing_(),
      ...
      samples_ring_buffer_(FFT_BUFFER_CHUNK_SIZE, 0),
...
Signature SignatureGenerator::GetNextSignature() {
  ...
  while (input_pending_processing_.size() - sample_processed_ >= 128 &&
    ...
    LowQualityTrack input(
        input_pending_processing_.begin() + sample_processed_,
...
void SignatureGenerator::processInput(const LowQualityTrack& input) {
  ...
  for (std::size_t chunk = 0; chunk < input.size(); chunk += 128) {
    LowQualityTrack chunk_input(input.begin() + chunk,
                                input.begin() + chunk + 128);
...
void SignatureGenerator::doFFT(const LowQualityTrack& input) {
  ...
  decltype(fft_object_)::FFTOutput real =
      fft_object_.RFFT(excerpt_from_ring_buffer);
...
void SignatureGenerator::doPeakSpreading() {
  ...
  for (auto position = 0u; position < decltype(fft_object_)::OUTPUT_SIZE;
       ++position) {
    ...
    for (auto former_fft_num : {-1, -3, -6}) {
      auto& former_fft_ouput =
          spread_ffts_output_[(spread_ffts_output_.position() +
                               former_fft_num) %
...
void SignatureGenerator::doPeakRecognition() {
  const auto& fft_minus_46 =
      fft_outputs_[(fft_outputs_.position() - 46) % fft_outputs_.size()];
  const auto& fft_minus_49 =
      spread_ffts_output_[(spread_ffts_output_.position() - 49) %
                          spread_ffts_output_.size()];

  auto other_offsets = {-53, -45, 165, 172, 179, 186, 193,
                        200, 214, 221, 228, 235, 242, 249};
  for (auto bin_position = 10u;
       bin_position < decltype(fft_object_)::OUTPUT_SIZE; ++bin_position) {
    if (fft_minus_46[bin_position] >= 1.0 / 64.0 &&
      ...
      if (fft_minus_46[bin_position] > max_neighbor_in_fft_minus_49) {
        ...
        if (fft_minus_46[bin_position] > max_neighbor_in_other_adjacent_ffts) {
          ...
          auto peak_magnitude =
              std::log(std::max(1.0l / 64, fft_minus_46[bin_position])) *
                  1477.3 +
              6144;
          auto peak_magnitude_before =
              std::log(std::max(1.0l / 64, fft_minus_46[bin_position - 1])) *
                  1477.3 +
              6144;
          ...
          band_to_sound_peaks[band].push_back(FrequencyPeak(
